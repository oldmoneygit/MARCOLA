/**
 * @file meta-actions-executor.ts
 * @description Executor dos Tools de Meta-Ação do MARCOLA Assistant
...
export class MetaActionsExecutor {
  ...
  async executarRotinaMatinal(params: {
    ...
    const tarefasUrgentes = tarefasFormatadas.filter(
      t => t.prioridade === 'urgent' || t.prioridade === 'high' || t.atrasada
    ).length;
    ...
    if (params.incluirSugestoes !== false) {
      if (vencidos.length > 0) {
    ...
    if (params.incluirMetricas !== false) {
  ...
  async encerrarDia(params: {
    ...
    // 3. Mensagens enviadas hoje (simplificado)
    ...
    if (params.incluirPreviewAmanha !== false) {
      ...
      const { count: reunioesAmanha } = await supabase
        .from('meetings')
        .select('*', { count: 'exact', head: true })
        .eq('user_id', this.userId)
        .eq('date', amanhaStr)
      ...
      const { count: tarefasAmanha } = await supabase
        .from('tasks')
        .select('*', { count: 'exact', head: true })
        .eq('user_id', this.userId)
      ...
      const { data: pagamentosAmanha } = await supabase
        .from('payments')
        .select('amount')
        .eq('user_id', this.userId)
        .eq('due_date', amanhaStr)
  ...
  async prepararOnboarding(params: {
    ...
    if (!sugestaoDataReuniao) {
      ...
      sugestaoDataReuniao = toDateString(proximoDiaUtil);
    }

    // Tarefas iniciais padrão
    const tarefasIniciais: OnboardingCliente['itensCriar']['tarefasIniciais'] = [
      {
    ...
    // Primeira cobrança
    let primeiraCobranca: OnboardingCliente['itensCriar']['primeiraCobranca'];
    if (params.valorMensal || cliente.monthly_value) {
      ...
      primeiraCobranca = {
        criar: true,
        valor,
        vencimento: toDateString(vencimento),
        descricao: 'Primeira mensalidade'
      };
    }

    // Lembretes
    const lembretes: OnboardingCliente['itensCriar']['lembretes'] = [
      {
    ...
    const checklistInfo: OnboardingCliente['checklistInfo'] = [
    ...
    const passos: OnboardingCliente['passos'] = [
      { ordem: 1, nome: 'Cadastro básico', descricao: 'Dados do cliente no sistema', status: 'concluido', obrigatorio: true },
      { ordem: 2, nome: 'Primeira reunião', descricao: 'Alinhamento inicial', status: 'pendente', obrigatorio: true },
      { ordem: 3, nome: 'Coleta de acessos', descricao: 'Obter credenciais necessárias', status: 'pendente', obrigatorio: true },
      { ordem: 4, nome: 'Configuração técnica', descricao: 'Pixel, conta, BM', status: 'pendente', obrigatorio: true },
      { ordem: 5, nome: 'Primeiras campanhas', descricao: 'Subir campanhas iniciais', status: 'pendente', obrigatorio: true }
    ];
  ...
  async executarHealthCheck(params: {
    ...
    const novosClientesMes = todosClientes?.filter(c =>
      c.created_at && new Date(c.created_at) >= dataInicio
    ).length || 0;
    ...
    // 4. Distribuição de Clientes por Saúde (simplificado)
